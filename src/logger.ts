...
import { Workflow, Step } from "workflow-core";
...
function pfx(type: string): string {
    const {
        runId = "-", method = "-", workflowName = "-", stepId = "-",
    } = als.getStore() ?? {};
    const methodCallName = resolveMethodCallName();
    return `[${new Date().toISOString()}] [${type}] [${runId}] [${method}] [${workflowName}] [${stepId}] [${methodCallName}]`;
}

export const log = {
    trace(msg: unknown, ...args: unknown[])  { if (level >= 4) console.debug(pfx("trace"), msg, ...args); },
    debug(msg: unknown, ...args: unknown[])  { if (level >= 3) console.debug(pfx("debug"), msg, ...args); },
    info(msg: unknown, ...args: unknown[])   { if (level >= 2) console.info (pfx("info"), msg, ...args); },
    warn(msg: unknown, ...args: unknown[])   { if (level >= 1) console.warn (pfx("warn"), msg, ...args); },
    error(msg: unknown, ...args: unknown[])  { if (level >= 0) console.error(pfx("error"), msg, ...args); },

    set(partial: Partial<Ctx>)      { als.enterWith({ ...(als.getStore() ?? {}), ...partial }); },
};
...
Workflow.prototype.run = function (this: Workflow<any, any>, trigger: unknown): any {
