...
log.info("OpenRouter API instances:", openAiInstances.size, "models:", thinkFreeModels.length);
...
export async function chatWithModel(systemPrompt: string, userPrompt: string, temperature: number, model: string) {
    for (const openai of openAiInstances) {
        try {
            return await chatWithModelAndInstance(systemPrompt, userPrompt, temperature, model, openai)
        } catch(err) {
            log.trace(err);
        }
    }
...
async function chatWithModelAndInstance(systemPrompt: string, userPrompt: string, temperature: number, model: string, openai: OpenAI) {
