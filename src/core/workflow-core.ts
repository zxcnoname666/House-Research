...
const __ = (() => {
    ...
    const isParallel = (v: unknown): v is ParallelDescriptor =>
        !!v && typeof v === "object" &&
        "parallel" in (v as Record<string, unknown>);

    type Last<T extends readonly any[]> = number extends T["length"] ? never
        : T extends readonly [...any, infer L] ? L
            : never;
    ...
    type MergeInputs<T extends readonly StepLike[]> = T extends
        readonly [infer F extends StepLike, ...infer R extends StepLike[]]
        ? InNode<F> & MergeInputs<R>
        : unknown;

    type MergeOutputs<T extends readonly StepLike[]> = T extends
        readonly [infer F extends StepLike, ...infer R extends StepLike[]]
        ? OutNode<F> & MergeOutputs<R>
        : unknown;

    type InNode<N> = N extends Step<any, any> ? In<N>
        : N extends ParallelDescriptor ? MergeInputs<N["parallel"]>
            : never;

    type OutNode<N> = N extends Step<any, any> ? Out<N>
        : N extends ParallelDescriptor ? MergeOutputs<N["parallel"]>
            : never;

    type ValidateChain<L extends readonly StepLike[]> = L extends readonly [
            infer A extends StepLike,
            infer B extends StepLike,
            ...infer R extends StepLike[],
        ] ? OutNode<A> extends InNode<B> ? ValidateChain<[B, ...R]> : never
        : L;

    type OutputOfSteps<S extends readonly StepLike[]> = [Last<S>] extends [never]
        ? unknown
        : OutNode<Last<S>>;
    ...
    > {
        ...
        public async run(
            ...
            const trigger = this.triggerSchema
                ? this.triggerSchema.parse(rawTrigger)
                : rawTrigger;
            const bag: Bag = {};
            ...
            const exec = async (node: StepLike): Promise<void> => {
                if (isParallel(node)) {
                    ...
                    base = results.reduce<Record<string, unknown>>(
                        (acc, cur) =>
