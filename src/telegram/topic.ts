...
export interface SendToTopicOptions {
    ...
    iconColor?: 16478047 | 7322096 | 16766590 |
        13338331 | 9367192 | 16749490 |
        undefined;               // один из шести RGB 0x6FB9F0 … 0xFB6F5F
}

export async function sendToTopic(bot: Bot, chatId: number | string, opts: SendToTopicOptions) {
    const {
        topicKey, title, message, quotes,
        imageUrls, lat, lon, address,
        placeTitle, iconColor,
    } = opts;
    ...
    try {
        ...
        await safeSendAlbumsToTopic(bot, chatId, threadId, imageUrls, title).catch(log.error);
        ...
        for (const chunk of [
            ...chunkText(message),
            ...quotes.map(text => chunkText(text, 4000)).flat().map(quote => `<blockquote expandable>${quote}</blockquote>`)
        ]) {
