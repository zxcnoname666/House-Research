...
import {chat as geminiChat, chatLite as geminiChatLite} from "#ai-agents/gemini.ts";
import { deadline } from "@std/async";
import {log} from "#logger";

const TIMEOUT = 240_000;

export async function safeFreeAi(systemPrompt: string, userPrompt: string, temperature: number = 0): Promise<string> {
    try {
...
export async function safeFreeThink(systemPrompt: string, userPrompt: string, temperature: number = 0): Promise<string> {
    try {
        return await deadline(openChat(systemPrompt, userPrompt, temperature), TIMEOUT);
    } catch(err) {
        log.trace(err);
