...
>({
    ...
    outputSchema: z.object({ message: z.string(), title: z.string(), address: z.string(), routes: z.string(), operators: z.string(), images: z.array(z.string()), ratingKey: z.string(), geo: z.object({ lat: z.number(), lon: z.number()}) }),
    async execute(_, bag) {
        log.debug("run step");
        ...
        log.debug("rating...");
        const finalMessage = await safeFreeThink(finalRewrite, `${result}\n\n## Операторы\n${operators}\n\n## Маршруты\n${routes}`);
        log.debug("finalMessage", finalMessage);
        const finalTitle = await safeFreeThink(titleMaker, `${title}\n\n${address}\n\n${result}\n\n## Операторы\n${operators}\n\n## Маршруты\n${routes}`);
