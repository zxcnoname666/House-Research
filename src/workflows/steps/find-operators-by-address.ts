...
async function getRawOperators(slug: string): Promise<string> {
  ...
  const text = doc.querySelector('div[class*="AddressTariff_currentAddress"]')
    ?.textContent.replace("изменить адрес", "");
  return text ?? "N/A";
}
...
async function searchAddress(address: string): Promise<string> {
  const res = await spyFetch(
    `https://bff.gdelu.ru/api/v1/suggest/addr?context=specify&term=${
      encodeURI(address)
    }`,
  );
