...
import {pasteFileAndFlashWebAsk, pasteFileAndLiteFlashWebAsk, uploadFilesAndChat} from "#ai-agents/gemini.ts";
...
export const parseAvitoAdFromUrl = new Step<
    { url: string },
    { mergedImage: string, images: string[], description: string, title: string, address: string }
>({
    id: "parseAd",
    inputSchema: z.object({ url: z.string() }),
    outputSchema: z.object({ mergedImage: z.string(), images: z.array(z.string()), description: z.string(), title: z.string(), address: z.string() }),
    execute: runStep,
});
...
async function runStep({url: urlString}: {url: string}): Promise<{mergedImage: string, images: string[], description: string, title: string, address: string}> {
...
async function runCore({html}: {html: string}): Promise<{mergedImage: string, images: string[], description: string, title: string, address: string}> {
    ...
    const title = doc.head.querySelector('meta[property="vk:title"]')?.getAttribute("content") ?? "N/A";
    const address = doc.querySelector('div[itemProp="address"]')?.textContent ?? "N/A";
    ...
    const aiParse = await pasteFileAndFlashWebAsk(avitoParser, "", {mimeType: "text/html", base64: htmlBase64}, 0)
            .catch(() => pasteFileAndLiteFlashWebAsk(avitoParser, "", {mimeType: "text/html", base64: htmlBase64}, 0))
        ?? "N/A";
...
if (import.meta.main) {
    const urlString = "https://www.avito.ru/moskva/kvartiry/1-k._kvartira_35_m_816_et._4212251529";
    console.log(await runStep({url: urlString}));
    const html = await (await spyFetch(urlString)).text();
    console.info(parsePhotos(html));
    const aiParse = await uploadFilesAndChat(avitoParser, "", [urlString], 0) ?? "N/A";
    console.log(aiParse)
}