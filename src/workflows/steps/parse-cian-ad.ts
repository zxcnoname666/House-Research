...
export const parseCianAdFromAgent = new Step<
    { exportUrl: string, images: string[], title: string, address: string, metadata: string },
    { mergedImage: string, images: string[], description: string, title: string, address: string }
>({
    id: "parseAd",
    inputSchema: z.object({ exportUrl: z.string(), images: z.array(z.string()), title: z.string(), address: z.string(), metadata: z.string() }),
    ...
    async execute({exportUrl, images, title, address, metadata}) {
        ...
        const description = await uploadFilesAndChat(readPdf, `[METADATA]${metadata}[/METADATA]`, [exportUrl.toString()], 0) ?? "N/A";

        return {mergedImage, images, description, title: title.split('\n')[0], address};
    },
});
...
async function runStep({url: urlString}: {url: string}): Promise<{mergedImage: string, images: string[], description: string, title: string, address: string}> {
    ...
    const exportUrl = new URL(`https://${url.host}${url.pathname.replace(/\/rent\/flat\//, "/export/pdf/rent/flat/")}`);
    const html = await (await spyFetch(urlString)).text();

    if (html.includes("Нам очень жаль, но запросы с вашего устройства похожи на автоматические.")) {
    ...
    const title = doc.querySelector('div[data-name="OfferTitleNew"]')?.textContent ?? "N/A";
    ...
    const description = await uploadFilesAndChat(readPdf, "", [exportUrl.toString()], 0) ?? "N/A";
