import { chain, createWorkflow, Parallel } from "workflow-core";
import {
  parseCianAdFromAgent,
  parseCianAdFromUrl,
} from "./steps/parse-cian-ad.ts";
...
export const cianParseFromUrl = createWorkflow({
  ...
  steps: chain(
    parseCianAdFromUrl,
    Parallel(findOperatorsByAddress, getNavigationFromAddress),
    cianFull(),
    ratingCalculation,
    computeFinalSchema,
  ),
...
if (import.meta.main) {
  const output = await cianParseFromUrl.run({
    url: "https://cian.ru/rent/flat/319554040/",
  });
